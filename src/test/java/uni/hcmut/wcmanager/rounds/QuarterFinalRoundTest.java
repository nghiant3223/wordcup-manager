...
public class QuarterFinalRoundTest {
    ...
    public void test_getResult(){
        ...
        QuarterFinalRound quarterFinalRound = new QuarterFinalRound(matchList);
        Map<Integer, Team[]> result = quarterFinalRound.getResult();
        Team[] quarterFinalRound1 =  result.get(0);
        Team[] quarterFinalRound2 =  result.get(1);
        Assert.assertEquals(2, result.size());
        Assert.assertEquals(TeamHome, quarterFinalRound1[0]);
        Assert.assertEquals(TeamHome2, quarterFinalRound2[0]);
    }

    @Test
    public void test_getResult_with_match_null(){

        List<Match> quarterFinalRoundMatch = new ArrayList<>();
        QuarterFinalRound quarterFinalRound = new QuarterFinalRound(quarterFinalRoundMatch);
        Map<Integer, Team[]> result =  quarterFinalRound.getResult();
    ...
    public void test_getName(){
        List<Match> quarterFinalRoundMatch = new ArrayList<>();
        QuarterFinalRound quarterFinalRound = new QuarterFinalRound(quarterFinalRoundMatch);
        Assert.assertEquals(RoundName.QUARTER_FINAL_ROUND, quarterFinalRound.getName());
    }
