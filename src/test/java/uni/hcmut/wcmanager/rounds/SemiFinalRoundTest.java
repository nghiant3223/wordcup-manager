...
public class SemiFinalRoundTest {
    ...
    public void test_getResult(){
        ...
        SemiFinalRound semiFinalRound = new SemiFinalRound(matchList);
        Map<Integer, Team[]> result = semiFinalRound.getResult();
        Team[] semiFinalRound1 =  result.get(0);
        Team[] semiFinalRound2 =  result.get(1);
        Assert.assertEquals(2, result.size());
        Assert.assertEquals(TeamHome, semiFinalRound1[0]);
        Assert.assertEquals(TeamHome2, semiFinalRound2[0]);
    }

    @Test
    public void test_getResult_with_match_null(){

        List<Match> semiFinal = new ArrayList<>();
        SemiFinalRound semiFinalMatch = new SemiFinalRound(semiFinal);

        Map<Integer, Team[]> result =  semiFinalMatch.getResult();
    ...
    public void test_getName(){
        List<Match> semiFinal = new ArrayList<>();
        SemiFinalRound semiFinalMatch = new SemiFinalRound(semiFinal);
        Assert.assertEquals(RoundName.SEMI_FINAL_ROUND, semiFinalMatch.getName());
    }
